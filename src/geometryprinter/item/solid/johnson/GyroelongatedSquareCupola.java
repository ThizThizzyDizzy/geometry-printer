...
public class GyroelongatedSquareCupola extends ItemFractured{
    private int size;
    public GyroelongatedSquareCupola(){
        this(0);
    }
    public GyroelongatedSquareCupola(int size){
        this.size = size;
    }
    @Override
    public String getDisplayName(){
        return size+" Pixel Gyroelongated Square Cupola";
    }
    @Override
    ...
    public boolean isEqual(Item item){
        if(item instanceof GyroelongatedSquareCupola){
             if(size==((GyroelongatedSquareCupola) item).size){
                 return true;
             }
        }
        return false;
    ...
    public PageObject replicate(){
        return new GyroelongatedSquareCupola(size);
    }
    @Override
    public void split(ArrayList<PageObject> split){
        ...
        split.add(new Polygon(8,size,true));
    }
    @Override
