...
public class TriangularHebesphenorotunda extends ItemFractured{
    private int size;
    public TriangularHebesphenorotunda(){
        this(0);
    }
    public TriangularHebesphenorotunda(int size){
        this.size = size;
    }
    @Override
    public String getDisplayName(){
        return size+" Pixel Triangular Hebesphenorotunda";
    }
    @Override
    ...
    public boolean isEqual(Item item){
        if(item instanceof TriangularHebesphenorotunda){
             if(size==((TriangularHebesphenorotunda) item).size){
                 return true;
             }
        }
        return false;
    ...
    public PageObject replicate(){
        return new TriangularHebesphenorotunda(size);
    }
    @Override
    public void split(ArrayList<PageObject> split){
        split.add(new EquilateralTriangleArray(size, 13));
        for(int i = 0; i<3; i++){
