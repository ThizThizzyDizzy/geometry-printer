...
public class SquareCupola extends ItemFractured{
    private int size;
    public SquareCupola(){
        this(0);
    }
    public SquareCupola(int size){
        this.size = size;
    }
    @Override
    public String getDisplayName(){
        return size+" Pixel Square Cupola";
    }
    @Override
    ...
    public boolean isEqual(Item item){
        if(item instanceof SquareCupola){
             if(size==((SquareCupola) item).size){
                 return true;
             }
        }
        return false;
